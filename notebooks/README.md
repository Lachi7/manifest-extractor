...
A Google Colab notebook that uses Google's Gemini vision-language model to extract structured data from scanned Russian-language railway shipping documents, converting messy scanned paperwork into clean, exportable spreadsheets (CSV/JSON).

Built at **ASCO CSJC** to replace the manual process of locating wagon numbers, container/waybill numbers, and ГНГ (cargo classification) codes across scanned PDF manifests and typing them into Excel by hand.

---

## Overview

Railway freight shipments generate several kinds of paper documents — dense multi-wagon manifest tables, ferry transfer summary tables, and single-wagon waybill forms — all typewritten or handwritten in Cyrillic, often scanned at odd rotations and varying quality. This notebook automates reading them:
...
| `other` | Anything that doesn't match the above — skipped, no extraction call made. |
...
PDF upload
   │
   ▼
pdf_to_images()          — converts PDF pages to rotated PIL images (via pdf2image/poppler)
   │
   ▼
classify_page(s)()       — Gemini call #1 (cheap, thinking_budget=0): tags each page's document_type
   │
   ▼
extract_page() / extract_gng_wagon()   — Gemini call #2 (per matching page): pulls structured fields
   │                                       using a Pydantic response_schema, temperature=0
   ▼
pandas DataFrames        — one per document type, assembled across all pages
   │
   ▼
CSV + JSON export        — saved to output_dir; optional merge of GNG codes onto wagon summary
```

Two independent extraction paths are provided:

- **`process_manifest()`** — for `transfer_manifest_table` and `wagon_summary_table` pages. Produces `*_transfer_manifest.csv`, `*_wagon_summary.csv`, plus header JSON files.
- **`extract_gng_wagon_list()`** — for `single_wagon_waybill` pages. Produces a wagon-number ↔ ГНГ-code list, which can then be left-merged onto the wagon summary table by `wagon_number`.
...
| `GNGWagonPair` | Two-field schema (wagon code + ГНГ code) used for single-wagon waybill pages. |
